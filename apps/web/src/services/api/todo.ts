...
export const readTodoById = async (id: string): Promise<ITodo | undefined> => {
	const response = await axios.get<IApiResponse<ITodo | undefined>>(
		`/todos/${id}`,
	);
	return response.data.data;
};
...
export const updateTodo = async (
	...
	const response = await axios.patch<IApiResponse<ITodo | undefined>>(
		`/todos/${id}`,
		updates,
	);
	return response.data.data;
...
export const deleteTodo = async (id: string): Promise<ITodo | undefined> => {
	const response = await axios.delete<IApiResponse<ITodo | undefined>>(
		`/todos/${id}`,
	);
	return response.data.data;
};
